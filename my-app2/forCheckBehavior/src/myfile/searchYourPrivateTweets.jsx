    ...
    if(yourStr.length > 0){
      if(yourStr.indexOf("AND")!==-1 && yourStr.indexOf("OR")!==-1){
        ...
        if(yourStr.indexOf("OR")===-1){
          ...
          const yourWords = yourStr.split("AND").filter((value)=>(value!==''));
        ...
        console.log("結果を取得しました ["+document.getElementById("yourWords").value+"]");
      }
    }else{
...
const SelectAndLoadFile = (props) => {
  ...
  return (
    <div>
      <input type="file" id="yourFile" onChange={()=>getTweets({setTweets: setTweets})} />
    </div>
  );
};
  ...
  return (
    <div>
      <form onSubmit={(e)=>{e.preventDefault();return false;}}>
        ...
        <button onClick={()=>GetResult({tweets:tweets, setResult:setResult})}>検索</button>
      </form>
    </div>
  );
};
...
const ShowResult = (props) => {
  ...
  React.useEffect(() => {
    if(result.length > 0){
      ...
      setElement(<div><div>検索結果</div><div>
        {result.map((tweet,key) => <TweetElement tweet={tweet} key={key} />)}
      </div></div>);
    }else{
      setElement(<div><div>検索結果</div>
      </div>);
    }
  }, [result]);
  return element;
